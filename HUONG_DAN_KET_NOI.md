...
| **QR — Ghép nối LAN** | Kết nối nhanh 1 lần, 2 máy ở gần nhau | TTBDebugPlus → Connection Health → quét QR bằng nút **Scan QR** trong Debug Bridge panel |
| **QR — Cấu hình Relay** *(mới)* | Khác mạng (làm từ xa/WFH), hoặc muốn tự kết nối mỗi lần mở app mà không cần nhập lại | TTBDebugPlus → Settings → Relay → quét QR 1 lần. Lưu vĩnh viễn — tự kết nối lại mỗi lần mở app sau, kể cả sau khi build lại |
