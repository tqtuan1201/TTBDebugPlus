# Connection Guide — iOS ↔ DebugKit

A quick reference for connecting your iOS app to DebugKit on Mac. For the full step-by-step integration (SPM, sending logs, etc.), see [README.md](README.md). This page is just about **connecting**.
...
4. Open **DebugKit** on your Mac, same Wi-Fi as your iPhone. Done — it connects automatically.
...
| **Manual Connect** | Bonjour blocked (corporate Wi-Fi, VPN) | Debug Bridge panel → **Enter IP** → type Mac's IP:port shown in DebugKit → Connection Health |
| **QR — LAN Pairing** | Quick one-time connect, both devices in the same room | DebugKit → Connection Health → scan the QR via **TTBaseDebugKit → SCAN QR CODE** (fastest) or **Debug Bridge → Scan QR** |
| **QR — Relay Config** *(new)* | Different networks (remote/WFH), or you want it to "just work" every launch without re-typing anything | DebugKit → Settings → Relay → scan once via **TTBaseDebugKit → SCAN QR CODE** or Debug Bridge → Scan QR. Saved permanently — reconnects automatically on every future launch, even after a rebuild |
...
Open DebugKit — each device in the sidebar shows a small colored icon:
...
| Device never appears in DebugKit | Not on the same Wi-Fi, or Server isn't running on Mac | Check both are on the same network; check DebugKit menu bar shows "Server Running" |
...
More detail: `README.md` → Troubleshooting section, or the in-app **Integration Guide** tab in DebugKit.
